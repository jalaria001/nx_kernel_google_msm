...
if [ -e $KERNELDIR/arch/arm/boot/zImage ]; then
	...
	# copy all needed to out kernel folder
        ./utilities/mkbootimg --kernel $KERNELDIR/out/zImage --cmdline 'console=ttyHSL0,115200,n8 androidboot.hardware=hammerhead user_debug=31 msm_watchdog_v2.enable=1 androidboot.selinux=permissive' --base 0x00000000 --pagesize 2048 --ramdisk_offset 0x02900000 --tags_offset 0x02700000 --ramdisk ramdisk.gz --output $KERNELDIR/out/boot.img
